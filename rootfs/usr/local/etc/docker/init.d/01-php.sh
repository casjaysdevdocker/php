...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202410091028-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  LICENSE.md
# @@ReadME           :  01-php.sh --help
# @@Copyright        :  Copyright: (c) 2024 Jason Hempstead, Casjays Developments
# @@Created          :  Wednesday, Oct 09, 2024 10:28 EDT
# @@File             :  01-php.sh
# @@Description      :
...
# root user name
root_user_name="${PHP_ROOT_USER_NAME:-}"
# root user password
root_user_pass="${PHP_ROOT_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Normal user info [password/random]
# normal user name
user_name="${PHP_USER_NAME:-}"
# normal user password
user_pass="${PHP_USER_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Load variables from config
# Generated by my dockermgr script
[ -f "/config/env/php.script.sh" ] && . "/config/env/php.script.sh"
# Overwrite the variabes
[ -f "/config/env/php.sh" ] && . "/config/env/php.sh"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Additional predefined variables
PHP_DEV_SERVER_START="yes"
PHP_DEV_SERVER_PORT="$SERVICE_PORT"
...
__pre_execute() {
	...
	if __is_dir_empty "$WWW_ROOT_DIR"; then
		printf '%s\n%s\n%s\n' "Welcome to $CONTAINER_NAME" '<br /><br />' '<?php phpinfo( ); ?>' >"$WWW_ROOT_DIR/index.php"
	fi
	# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__post_execute() {
	...
	(
		...
		# commands to execute
		__cd "$WWW_ROOT_DIR" && __start_php_dev_server "$WWW_ROOT_DIR" "$PHP_DEV_SERVER_START" &
		# show exit message
